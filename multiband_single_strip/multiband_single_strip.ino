...
void loop() {
  ...
  if (n_Count < n_Samples){  
    ...
    Read_Frequencies();
    Scale_Band(Frequencies_One, 8, Scaled_Channel);
    for (int k = 0; k < 7; k++){
    ...
    for (int jjj = 0; jjj <7; jjj++){
      Light(new_average, jjj, (color*jjj + 32), width, width + 8) ;
      width = width + 8;
...
void Scale_Band(int* channel, int width, int* Scaled_Channel){
  for (int band=0; band<7; band++){
    float temp1=0.0;
    if (channel[band] > 70){
      temp1 = (float)channel[band];
      temp1 = (temp1/1000.0)*width;
      Scaled_Channel[band] = (int)temp1;
//      Serial.print(temp1);
//      Serial.print('\t');
...
//      Serial.print('\n');
  ...
  Print_Frequencies(Scaled_Channel);
} 
// You simply need to address i being always greater than your scaledd band
// add a second loop to go through each set of 8 leds at a time.
// This function actually lights up all of the leds in a single array
void Light(float* Scaled_Channel, int band_interval, int color, int start_led, int stop_led){
